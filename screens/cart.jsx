...
export const Cart = () => {
  ...
  const confirmOrder = async () => {
    try {
      const order = { items, total, userId: user.localId, date: new Date().toISOString() };
      await triggerPost(order).unwrap();
      setConfirmationMessage('Confirmación del pedido lista');
    } catch (error) {
      setConfirmationMessage('Lo lamentamos, el pedido no pudo ser confirmado');
    }
  };
